...
const generateToken = (id: string, subActive: boolean) => {
    return jwt.sign(
        { id, sub_active: subActive },
           ...
           try {
               ...
               if (!userResult) {
                   if (action === "signup") {
                       ...
                       console.log(
                           `Registered new user: ${email} via ${provider}`
                       )
               ...
               if (!user.subscription?.stripeCustomerId) {
                   ...
                   const classroom =
                       await this.classroomUseCase.findByTeacherId(
                           user._id.toString()
                       )
